...
- Name: LoginUsingDataDriven.spec.ts >> Login test with JSON Data: Invalid login
- Location: tests\LoginUsingDataDriven.spec.ts:13:9
        ...
        - generic [ref=f2e87]:
          ...
          - generic [ref=f2e91]:
            ...
            - generic [ref=f2e95]:
              ...
              - textbox "Password" [ref=f2e97]: AWERTY
...
```ts
  ...
  7  | 
  8  | //Load the Json
  9  | const JsonPath = 'data/logindata.json'
  10 | const JsonTestData = DataProvider.getTestDataFromJson(JsonPath)
  11 | 
  12 | for (const data of JsonTestData) {
  13 |     test(`Login test with JSON Data: ${data.testName}`, async ({ page }) => {
  14 |         let config = new TestConfig();
  15 |         await page.goto(config.appurl)
  16 | 
  17 |         let homePage = new HomePage(page)
  18 |         await homePage.clickMyAccount()
  19 |         await homePage.clickLogin()
  20 | 
  21 |         let loginPage = new LoginPage(page)
  22 |         await loginPage.login(data.email,data.password)
  23 | 
  24 |         await page.waitForTimeout(5000)
  25 | 
  26 |         let  myAccountPage = new MyAccountPage(page)
> 27 |         expect(await myAccountPage.isMyAccountPageExists()).toBeTruthy()
     |                                                             ^ Error: expect(received).toBeTruthy()
  28 | 
  29 | 
  30 | 
